...
TwoVector rule1(Square* b, Square bj, int n)
{

	TwoVector PCj = { 0 }; // "perceived center of mass"

	for (int i = 0; i < n; i++)
	{
		if (b[i].p.x != bj.p.x && b[i].p.y != bj.p.y)
		{
			PCj = Vector_Add(PCj, b[i].p);
		}
	...
	return Vector_Div(Vector_Sub(PCj, bj.p), 100);
}

TwoVector rule2(Square* b, Square bj, int n)
{
	...
	for (int i = 0; i < n; i++)
	{
		if (b[i].p.x != bj.p.x && b[i].p.y != bj.p.y)
		{
			...
			if (Abs_value(Vector_Sub(b[i].p, bj.p)) < 50)
			{
				c = Vector_Sub(c, Vector_Sub(b[i].p, bj.p));
			}


		}
	}
	return Vector_Div(c, 100);
}
TwoVector rule3(Square* b, Square bj, int n)
{
	TwoVector PVj = { 0 }; // Perceived velocity

	for (int i = 0; i < n; i++)
	{
		if (b[i].p.x != bj.p.x && b[i].p.y != bj.p.y)
		{
			PVj = Vector_Add(PVj, b[i].v);

		}
	}
	PVj = Vector_Div(PVj, n - 1);
	return Vector_Div(Vector_Sub(PVj, bj.v), 100);

}

void move_to_new_positions(Square* b, int n)
{
	TwoVector v1, v2, v3;
	...
	for (int i = 0; i < n; i++)
	{
		...
		v2 = rule2(b, b[i], n);
		v3 = rule3(b, b[i], n);
		b[i].v = Vector_Add(b[i].v, Vector_Add(v3, Vector_Add(v1, v2)));
		b[i].p = Vector_Add(b[i].p, b[i].v);
	}
	...
	for (;;)
	{
		for (int ii = 0; ii < max_n_squares; ii++)
		{
			...
			*/
			...
			v1 = rule1(b, b[ii], max_n_squares);
			v2 = rule2(b, b[ii], max_n_squares);
			v3 = rule3(b, b[ii], max_n_squares);
